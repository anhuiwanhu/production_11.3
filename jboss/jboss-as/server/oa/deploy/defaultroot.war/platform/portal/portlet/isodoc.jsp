...
String windowHeight = (confMap.get("windowHeight")!=null&&!"auto".equals(confMap.get("windowHeight")))?confMap.get("windowHeight"):"150";
//int height = Integer.valueOf(windowHeight)-20;
String hasTitle = confMap.get("hasTitle");
String hasToolbar = confMap.get("hasToolbar");
String hasBorder = confMap.get("hasBorder");
String imgNum = confMap.get("imgNum")!=null&&!confMap.get("imgNum").equals("")?confMap.get("imgNum"):"5";
...
var jsonData = [
	{
		...
		%>	
            ...
            for(int i0=0; i0<mapSize; i0++){
                ...
                String linkUrl = "";
                if("info".equals(moreType[0])){
                    linkUrl = "javascript:"+moreType[1];
                }else{
                    linkUrl = moreType[1];
                }
                if(_outter_)linkUrl="openWin({url:'"+rootPath+"/PortalInformation!informationList.action?channelId="+cid[i0]+"&iso=1', isFull:true});";
        	%>
			{title:"<%=className%>", url:"javascript:void(0)", onclick:"<%=linkUrl%>", defaultSelected:"<%=i0==0?"on":""%>",liCss:"wh-portal-overflow", morelink:"<%=linkUrl%>"},
			<%}%>
		<%}%>	
		]
	}
...
<%
            if(topPreview && ("0".equals(infoType) || "1".equals(infoType))){
                ...
                if("true".equals(ivo.getIsConf())){
                    _link = "openWin({url:'"+ivo.getLink()+"&iso=1',winName:'info"+ivo.getId()+"',isFull:true});"; 
                    if(_outter_)_link="openWin({url:'"+rootPath+"/PortalInformation!getInformation.action?title="+ivo.getPoptitle()+"&id="+ivo.getId()+"&iso=1', isFull:true});";
                }else{
        ...
        %>
				<div class="wh-portal-info-title">
					...
					<div class="wh-portal-infocon">
                         <p><%=content.length()>100?content.substring(0,100)+"…":content%><span><%=ivo.getTime()%></span><a href="javascript:void(0)" onclick="<%=_link%>">[详细]</a></p>
                    </div>
                </div>
<%}%>
...
if(isImage){
	ItemVO ivo = (ItemVO)list.get(0);
	String[] _images = (String[])imageMap.get(cid[i0]);
	String imageLink = "openWin({url:'"+_images[2]+"&iso=1',winName:'info"+ivo.getId()+"', isFull:true});";
	if(_outter_)
		imageLink="openWin({url:'"+rootPath+"/PortalInformation!getInformation.action?title="+_images[1]+"&id="+ivo.getId()+"&iso=1', isFull:true});";
%>
	<div class="wh-portal-pic-content clearfix">
...
for (int i = 0; i < listInfo.size(); i++) {
	...
	for (int k = 0; k < listAcc.size(); k++,c++) {	
		...
		if (((Integer) objAcc[4]).intValue() == 1) {
			...
			imageName=(smartInUse==1?rootPath+"/upload/isodoc/"+datePath+"/"+imageName:fileServer+"/upload/isodoc/"+subFolder+imageName);
			//
			...
			imageNewsTitleLink =
					...
					"信息管理&iso=1";
            if(_outter_){
                imageNewsTitleLink = rootPath+"/PortalInformation!getInformation.action?title="+imageNewsTitle+"&id="+obj[1]+"&iso=1";
            }
...
if(files.equals("")){
	...
	if(obj != null){
		...
		String imageNewsTitleLinkTmp =
					...
					"信息管理&iso=1";
            if(_outter_){
                imageNewsTitleLinkTmp = rootPath+"/PortalInformation!getInformation.action?title="+imageTitleTmp+"&id="+obj[1]+"&iso=1";
            }
				...
				%>
					 <li>
                		<div class="wh-portal-pic-slide-box" id="test-<%=fileArray.length%>">
                    		<a href="javascript:void(0)" class="wh-portal-pic-slide-img"><img name="image" style="cursor:pointer;" src="<%=fileArray[i]%>" onclick="openWin({url:'<%=linkArray[i]%>',winName:'info',isFull:true});" alt="<%=textArray[i]%>" /></a>
