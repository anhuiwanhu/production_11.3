                ...
                for(int k = 0; k < commentList_w.size(); k ++){
					...
					if(s_orgId!=null&&!s_orgId.equals("")&&!s_orgId.equals("null")){
						if(dealOrgIdMap.get(s_orgId+"_"+now_activityId+"_"+commentField)==null){
							 ...
							 eachOrgName2="<strong style='font-size:14px;'>"+s_orgName+"</strong><br />";
...
<%}%>
 ...
 <%if(handSignInUse_w==1){%>
    cur_commentFieldHtml +=
					...
					"<div valign=bottom align=\"right\"><button class=btnButton4font onclick=\"if (!<%=cur_commentField%>_SendOut.OpenSignature()){alert(<%=cur_commentField%>_SendOut.Status);};return false;\"><!-- 签章 --><%=Resource.getValue(localcom_comment,"workflow","workflow.Signature")%></button></div>"+				"<div>"+
