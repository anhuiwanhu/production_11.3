...
<table width="100%" border="0" cellpadding="2" cellspacing="0" class="Table_bottomline">
    ...
    <tr>
        <td>&nbsp;</td>
        <td>
            <input type="checkbox" name="userPO.isPasswordRule" id="isPasswordRule" value="1" <s:if test="isPasswordRule == 1">checked</s:if> onclick="javascript:changePasswordRule();"/>启用密码规则
            <div id="passwordRule_TR" style="display:<s:if test="isPasswordRule != 1">none</s:if>">密码规则：数字+字母+特殊字符（大于等于7位），要求每三个月至少更新一次，且两年内不能和以前的密码重复。系统提前两周提醒。如果没有修改，则每次登录时都会提示剩余天数。如果到期仍不修改，则系统自动休眠此用户。</div>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <input onclick="checkMobile(this)" type="checkbox" name="userPO.mobileUserFlag" <s:if test="userPO.mobileUserFlag == 1">checked</s:if> id="mobileUserFlag" value="1"/>
    ...
    <tr>
        ...
        <td>
            <select id="empDuty" name="empDuty" class="xeasyui-combobox" data-options="forceSelection:true,width:300">
                ...
                <option value="<s:property value="#request.listDuty[#status.index][1]"/>" <s:if test="#request.listDuty[#status.index][1] == userPO.empDuty">selected</s:if><s:elseif test="empId==null && #status.last">selected</s:elseif>><s:property value="#request.listDuty[#status.index][1]"/></option>
                </s:iterator>
            </select>
        </td>
    </tr>
        ...
        <td width="80%" valign="top">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td width="100" nowrap>
                        <select id="changeRights" class="xeasyui-combobox" data-options="panelHeight:'auto',width:200,onSelect: function(record){ChangeSelect(record);}">
	...
	$('#ImgShow').attr("src", "<%=path%>/upload/peopleinfo/"+json.save_name+json.file_type).load(function() {
        ...
        if(flag == false){            
            ...
            if(99>fileOffsetWidth || 19>fileOffsetHeight){
                whir_confirm("系统推荐上传的图片尺寸为100X20，而您上传的图片大小为"+fileOffsetWidth+"X"+fileOffsetHeight+"，可能导致显示不正常！您确定重新上传吗？", function(){
                    clearImg();
                },
                function(){
                });
