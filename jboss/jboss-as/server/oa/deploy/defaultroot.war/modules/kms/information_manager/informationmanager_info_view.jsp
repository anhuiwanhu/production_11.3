		...
		<tr valign="top">
			<td height="100%"><div class="docbox doc_contentbg">
				...
				<div class="news_contentBox">
					...
					<DIV class='news_redbox'>
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<div class="news_redright">	<img src="<%=rootPath%>/themes/common/images/icon_d1.gif" /><span class="operate_span" onclick="browser();"><s:text name="info.ReadCircumstance"/></span>
									<s:if test="information.informationOrISODoc!=1">
					...
					<IFRAME name='relationIFrame' id='relationIFrame' src='<%=rootPath%>/relation!relationIncludeList.action?moduleType=information&infoId=<s:property value="information.informationId"/>&showAdd=0&tagName=relationObjectDIV&iframeName=relationIFrame&relationview=1' frameborder='0' marginwidth='0' marginheight='0' scrolling='auto' width='100%' height='150' style="display:''"></IFRAME>	
					</s:if>
					<s:else>
					<div class="news_aboutFile">
						<table width="98%" border="0" align="center" cellpadding="1" cellspacing="1">
		...
		success:function(responseText){
			...
			for (var i=0; i<data.length; i++) {
				...
				if(po.commentIssuerId==userId || delComment=='1' || canVindicate=='true'){
					comment += '<div class="newsBtnbox"><span style="cursor:pointer;" onclick="modifyComment('+po.commentId+');"><img style="cursor:hand" border="0" src="<%=rootPath%>/images/modi.gif" title="<s:text name="info.columnmodify"/>"></span><span style="cursor:pointer;" onclick="deleteComment('+po.commentId+')"><img style="cursor:hand" border="0" src="<%=rootPath%>/images/del.gif" title="<s:text name="info.columndelete"/>"></span>&nbsp;</div>'
				}
...
function deleteComment(commentId){
	whir_confirm('<s:text name="info.deleteThisComment" />',function(){
		ajaxOperate({urlWithData:'Information!deleteComment.action?commentId='+commentId,tip:'<s:text name="info.DelComment" />',isconfirm:false,formId:'commentForm',callbackfunction:commentCallBack});
	});
}
