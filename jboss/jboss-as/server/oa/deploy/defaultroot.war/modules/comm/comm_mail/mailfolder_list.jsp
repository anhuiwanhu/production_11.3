...
<body class="MainFrameBox">
	<s:form name="queryForm" id="queryForm" action="innerMail!getMailfolderlist.action" method="post" >

	<!-- SEARCH PART START -->
	<%@ include file="/public/include/form_list.jsp"%>
    <input name="boxId" id="boxId" type="hidden" value="<%=boxId%>">
   ...
   //*************************************下面的函数属于公共的或半自定义的*************************************************//  
    ...
    function showTitle(po,i){
		...
		if(po.encrypt==null || po.encrypt==0){
            ...
            html += '<a href="javascript:void(0)" onclick="alertr();">'+po.mailsubject+'</a>'; 
		}
		return html;
	}
	function view(mailuserid,mailid,index){
	    ...
	    openWin({url:'innerMail!viewMail.action?mailuserid='+mailuserid+'&mailid='+mailid+'&index='+index+'&recordCount='+recordCount+'&boxId=<%=boxId%>&frommod=mailfolderlist',isFull:'true',winName: 'viewmail'+mailid });
	}
	function alertr() {
	...
	function delMailToDeserted() {
		 ajaxBatchOperate({url:"innerMail!delMailToDeserted.action",checkbox_name:"mailuserid",attr_name:"mailuserid",tip:'将这些邮件删入废件箱',isconfirm:true,formId:"queryForm",callbackfunction:null});
	}
	function delMails() {
