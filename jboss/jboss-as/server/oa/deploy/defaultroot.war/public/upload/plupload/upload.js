			...
			if(r_arr[i] != ''){		
				...
				if(canModify == 'yes'){
                    ...
                    if(',.jpg,.png,.gif,.bmp,.jpeg,'.indexOf(_fileType)!=-1){
						...
						if(',.txt,.pdf,.doc,.docx,.ppt,.pptx,'.indexOf(_fileType)!=-1){
                    ...
                    if(',.jpg,.png,.gif,.bmp,.jpeg,'.indexOf(_fileType)!=-1){
                        ...
                        _files_tr += '<td nowrap><a href="javascript:void(0);" onclick="window.open(\''+preUrl +'/public/download/player.jsp?fileType='+_fileType+'&fileName='+s_arr[i]+'&path='+dir+'\',\'\',\'TOP=100,LEFT=100,scrollbars=yes,resizable=yes,width=600,height=400\')">播放</a></td>';
                    }else if(isAttachPreview == '1' && ',.txt,.pdf,.doc,.docx,.ppt,.pptx,'.indexOf(_fileType)!=-1 && isPad()==false){//在线预览
                        _files_tr += '<td nowrap>&nbsp;<a href="javascript:void(0);" onclick="_previewFile(\''+previewUrl +'\', \''+dir+'\', \''+s_arr[i]+'\', \''+file_info.dlcode+'\', \''+isEncrypt+'\');">预览</a>';
                        _files_tr += '</td>';
                    }else  if(isAttachPreview == '1' &&',.xls,.xlsx,'.indexOf(_fileType)!=-1 && isPad()==false){
                         _files_tr+= '<td nowrap>&nbsp;<a href="javascript:void(0);" onclick="window.open(\''+whirRootPath+'/public/iWebOfficeSign/AttachmentEdit.jsp?moduleType='+dir+'&docName='+s_arr[i]+'&showEditButton=0&code='+file_info.dlcode+'&isEncrypt='+isEncrypt+'\',\'oneditfile\',\'TOP=100,LEFT=100,scrollbars=yes,resizable=yes,width=800,height=600\')">预览</a>';
                         _files_tr += '</td>';
...
function deleteUploadedFile(obj, dir, uniqueId, realFileNameInputId, saveFileNameInputId, isInit){
	window.parent.whir_confirm(comm.whir_upload_8, function(){
		...
		if(id == undefined || id == 'img'){
            ...
            var addParam = '';
            if(attachPortletSettingId != ''){//portlet设置上传图片、媒体文件，删除功能
                isInit = 1;
